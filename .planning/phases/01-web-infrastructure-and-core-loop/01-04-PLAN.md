...
must_haves:
  truths:
    ...
    - "Backend API responds within 100ms for WebSocket commands"
  artifacts: []
  key_links: []
---

<objective>
Verify complete end-to-end training flow with real-time visualization, parameter control, and performance requirements.
...
Output: Confirmation that all success criteria from ROADMAP Phase 1 are satisfied, including performance benchmarks.
</objective>
...
<task type="checkpoint:human-verify" gate="blocking">
  <what-built>
    ...
    - Canvas-based real-time grid visualization at 10+ fps
    - Parameter controls with presets and persistence
    - WebSocket bidirectional communication with <100ms latency
  </what-built>
  <how-to-verify>
    **Setup:**
    ...
    **Test Scenario 1: Basic Training Flow**
    ...
    2. Verify parameter controls:
       ...
       - Save/Load Q-table buttons present
    ...
    **Test Scenario 4: Q-table Persistence (QL-05)**
    1. Start training with default parameters
    2. Let train for 20-30 episodes
    3. Click "Save Q-table"
    4. Verify "Q-table saved" notification appears
    5. Click "Reset" to clear current state
    6. Click "Load Q-table"
    7. Verify "Q-table loaded" notification appears
    8. Start training again
    9. Observe agent behavior shows learned Q-values (takes shorter paths)
    10. Check .qtables/agent_latest.json exists and contains valid JSON

    **Test Scenario 5: Learning Verification**
    1. Start training with default parameters
    ...
    **Test Scenario 6: Performance Verification (WEB-04, VIZ-04)**
    1. Open DevTools Network tab, filter to WS (WebSocket)
    2. Click WebSocket connection, switch to Messages subtab
    3. Start training
    4. Measure time between training_update messages:
       - Should be ~100ms (10 Hz) ± 20ms tolerance
       - **WEB-04 verification:** Click Stop button, measure time from click to training halt
       - Should be < 100ms (near-instant response)
    5. Use browser Performance profiler:
       - Record for 5 seconds during active training
       - Check FPS in timeline: should maintain 60fps canvas render
       - Check for dropped frames or jank
    6. Verify smooth animation without stuttering

    **Test Scenario 7: Visual Quality**
    ...
    **Expected outcomes:**
    ...
    - **WEB-04:** WebSocket commands respond in < 100ms
    - **VIZ-04:** Visualization updates at 10+ Hz consistently
    - **QL-05:** Q-table saves and loads correctly across sessions
  </how-to-verify>
  <resume-signal>
    ...
    - Slow updates (< 10 Hz) → check broadcast timing in backend
    - Q-table not saving → check .qtables/ directory and file contents
  </resume-signal>
</task>
...
7. Visualization updates at 10+ fps (VIZ-04) ✓
8. Q-table persists across training sessions (QL-05) ✓
</verification>

<success_criteria>
Checkpoint passes when:
1. All 8 test scenarios pass without critical issues
...
5. Performance requirements met (WEB-04: <100ms, VIZ-04: 10+ Hz)
6. Q-table persistence works correctly (QL-05)
7. No major bugs or crashes during extended testing
8. System is ready for Phase 2 (learning metrics dashboard)
</success_criteria>
